...
class Restormer(nn.Module):
    ...
    def forward(self, inp_img):
        ...
        inp_dec_level3 = self.up4_3(latent) # 192 channels
        inp_dec_level3 = torch.cat([inp_dec_level3, out_enc_level3], 1) # 192*2 channels
        inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3) # 192 channels
        out_dec_level3 = self.decoder_level3(inp_dec_level3) # 192 channels

        inp_dec_level2 = self.up3_2(out_dec_level3) # 96 channels
        inp_dec_level2 = torch.cat([inp_dec_level2, out_enc_level2], 1) # 96*2 channels
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2) # 96 channels
        out_dec_level2 = self.decoder_level2(inp_dec_level2) # 96 channels

        inp_dec_level1 = self.up2_1(out_dec_level2) # 48 channels
        inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1) # 96 channels
...
# keras_model = get_visualkeras_model(model)

with open('Restormer summary.txt', 'w') as f:
    sys.stdout = f
    summary(model, (3, 224, 224))  # Adjust the input shape as needed
